...
class AddRemoteStorageUseCase(private val repo: RemoteStorageRepository) {
    suspend operator fun invoke(name: String, address: String, userId: String, password: String) {
        repo.insert(RemoteStorageItem(name = name, address = address, userId = userId, password = password))
    }
}
