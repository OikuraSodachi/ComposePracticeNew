@file:JvmName("LocalFileActionModelKt")
...
fun compressFilesRecursivelyToZip_td(files: Array<File>, zipFile: File) {
    ...
    fun addToZip(file: File, parentPath: String = "") {
        ...
        if (file.isFile) {
            ...
            file.listFiles()?.forEach { childFile -> addToZip(childFile, entryName) }
        }
    }
    for (file in files) { addToZip(file) }
...
fun compressFilesRecursivelyWithZip4j_td(files: Array<File>, zipFile: File) {
    ...
    for (file in files) {
        if (file.isFile) zipFileObject.addFile(file, parameters)
        else if (file.isDirectory) zipFileObject.addFolder(file, parameters)
    }
}
...
/** 진행상황 callback만 미완성 — unzipHere에 해당 */
@Throws(IOException::class)
...
fun zip4j_td() {
    ...
    val files = Arrays.asList(File("file1.txt"), File("file2.txt"))
